...
(function ($, undefined) {
    ...
    function _Init(originalTarget, options) {
        ...
        if (settings.draggable) {
            if (settings.nativeDrag) {
                popupWindow
                    ...
                    .on("mousedown touchstart", ".popupwindow_titlebar_draggable", _Native_Drag_Handle);
            } else {
                popupWindow
                    .on("mousedown touchstart", ".popupwindow_titlebar_draggable", _Titlebar_MouseDown);
            }
        }
        ...
        if (settings.buttons.close) {
            $("<div>", {
                ...
                .on("click touchstart", _ButtonClose_Click)
                .append(_icons.close)
                .appendTo(titlebar);
        }
        if (settings.buttons.maximize) {
            $("<div>", {
                ...
                .on("click touchstart", _ButtonMax_Click)
        ...
        if (settings.buttons.collapse) {
            $("<div>", {
                ...
                .on("click touchstart", _ButtonCollapse_Click)
        ...
        if (settings.buttons.minimize) {
            $("<div>", {
                ...
                .on("click touchstart", _ButtonMin_Click)
    ...
    function _AddDocumentMouseEventHandlers(eventData) {
        ...
        $(document)
            .on("mousemove touchmove", eventData, _Document_MouseMove)
            .on("mouseup touchend touchcancel", eventData, _Document_MouseUp);
    }
    ...
    function _Titlebar_MouseDown(event) {
        ...
        _AddDocumentMouseEventHandlers({
            ...
            compensationX: (event.originalEvent.touches ? event.originalEvent.touches[0].pageX : event.pageX) - currentPosition.left,
            compensationY: (event.originalEvent.touches ? event.originalEvent.touches[0].pageY : event.pageY) - currentPosition.top
        });
    ...
    function _Resizer_MouseDown(event) {
        ...
        _AddDocumentMouseEventHandlers({
            ...
            startX: (event.originalEvent.touches ? event.originalEvent.touches[0].pageX : event.pageX) + ((event.data.directionX == "left") ? currentSize.width : -currentSize.width),
            startY: (event.originalEvent.touches ? event.originalEvent.touches[0].pageY : event.pageY) + ((event.data.directionY == "top") ? currentSize.height : -currentSize.height),
            compensationX: (event.originalEvent.touches ? event.originalEvent.touches[0].pageX : event.pageX) - currentPosition.left,
            compensationY: (event.originalEvent.touches ? event.originalEvent.touches[0].pageY : event.pageY) - currentPosition.top
        });
        event.preventDefault();
    ...
    function _Document_MouseMove(event) {
        ...
        switch (event.data.action) {
            case "drag":
                newPosition.top = (event.originalEvent.touches ? event.originalEvent.touches[0].pageY : event.pageY) - event.data.compensationY;
                newPosition.left = (event.originalEvent.touches ? event.originalEvent.touches[0].pageX : event.pageX) - event.data.compensationX;
                if (settings.keepInViewport) {
                    var size = _GetCurrentSize(popupWindow);
                    var $window = $(window);
                    if (newPosition.top < 0) newPosition.top = 0;
                ...
                break;
            case "resize":
                if (event.data.dimension != "height" && (event.originalEvent.touches ? event.originalEvent.touches[0].pageX : event.pageX) > 0) {
                    var newWidth = (event.data.directionX == "left") ? event.data.startX - (event.originalEvent.touches ? event.originalEvent.touches[0].pageX : event.pageX) : (event.originalEvent.touches ? event.originalEvent.touches[0].pageX : event.pageX) - event.data.startX;
                    if (newWidth >= settings.minWidth && (!settings.maxWidth || newWidth <= settings.maxWidth)) {
                        newSize.width = newWidth;
                        if (event.data.directionX == "left") newPosition.left = (event.originalEvent.touches ? event.originalEvent.touches[0].pageX : event.pageX) - event.data.compensationX;
                    }
                }
                if (event.data.dimension != "width" && (event.originalEvent.touches ? event.originalEvent.touches[0].pageY : event.pageY) > 0) {
                    var newHeight = (event.data.directionY == "top") ? event.data.startY - (event.originalEvent.touches ? event.originalEvent.touches[0].pageY : event.pageY) : (event.originalEvent.touches ? event.originalEvent.touches[0].pageY : event.pageY) - event.data.startY;
                    if (newHeight >= settings.minHeight && (!settings.maxHeight || newHeight <= settings.maxHeight)) {
                        newSize.height = newHeight;
                        if (event.data.directionY == "top") newPosition.top = (event.originalEvent.touches ? event.originalEvent.touches[0].pageY : event.pageY) - event.data.compensationY;
                    }
                }
                break;
        }
    ...
    function _Document_MouseUp(event) {
        ...
        $(document)
            .off("mousemove touchmove", _Document_MouseMove)
            .off("mouseup touchend touchcancel", _Document_MouseUp);
        if (!settings.modal) popupWindow.data("overlay").width(0).height(0).css("background-color", "");
